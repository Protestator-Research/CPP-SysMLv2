...
void SysMLv2ListenerImplementation::exitFeature(SysMLv2Parser::FeatureContext* ctx) {
	...
	{
		if (ctx->feature_prefix()->feature_direction() != nullptr)
		{
			if (ctx->feature_prefix()->feature_direction()->KEYWORD_IN() != nullptr)
				f->setDirection(KerML::Entities::FeatureDirectionKind::IN);
			if (ctx->feature_prefix()->feature_direction()->KEYWORD_INOUT() != nullptr)
				f->setDirection(KerML::Entities::FeatureDirectionKind::IN_OUT);
			if (ctx->feature_prefix()->feature_direction()->KEYWORD_OUT() != nullptr)
	...
	if (ctx->feature_declaration() != nullptr)
	{
		...
		if (ctx->feature_declaration()->feature_specialization_part() != nullptr)
		{
			const auto fspecctx = ctx->feature_declaration()->feature_specialization_part();
			std::vector<std::shared_ptr<KerML::Entities::Specialization>> ownedSpecialisations;
			for (const auto& featureSpec : fspecctx->feature_specialization()) {
				if (featureSpec->typings() != nullptr) {
					const auto& elemName = featureSpec->typings()->typed_by()->owned_feature_typing()->general_type()->qualified_name()->getText();
					const auto elem = std::dynamic_pointer_cast<KerML::Entities::Specialization>(findElementWithName(elemName));
					ownedSpecialisations.push_back(elem);
					for (const auto additionalTypings : featureSpec->typings()->owned_feature_typing())
					{
						const auto& elemName2 = additionalTypings->general_type()->qualified_name()->getText();
						const auto elem2 = std::dynamic_pointer_cast<KerML::Entities::Specialization>(findElementWithName(elemName2));
						ownedSpecialisations.push_back(elem2);
					}
				}
			}
			f->setOwnedSpecialization(ownedSpecialisations);
		}
	}
